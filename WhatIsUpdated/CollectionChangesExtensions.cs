...
namespace Carable
{
    public static class CollectionChangesExtensions
    {
        ...
        public static CollectionChanges<T> CollectionChanges<T, TKey>(this IEnumerable<T> existing, IEnumerable<T> updated, Func<T, TKey> keySelector)
        {
            var e = existing.ToArray();
            var u = updated.ToArray();
            var dicE = existing.ToDictionary(keySelector, Id);
            var dicU = updated.ToDictionary(keySelector, Id);
